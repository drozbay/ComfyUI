...
class MiniMaxH3Model(nn.Module):
    ...
    def _forward(self, x, timestep, context, transformer_options={}, minimax_payload=None, denoise_mask=None, audio_denoise_mask=None, **kwargs):
        ...
        t_pin_v = max(t_v, VISUAL_COND_TIMESTEP) # pinned video timestep for preserved regions
        t_pin_a = max(t_a, AUDIO_COND_TIMESTEP) # pinned audio timestep for preserved regions
        video_w = None
        audio_w = None
